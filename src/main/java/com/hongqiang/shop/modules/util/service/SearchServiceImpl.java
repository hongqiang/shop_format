  ...
  implements SearchService
{
  ...
  public void index(Class<?> type)
  {
    ...
    if (type == Article.class)
      ...
      {
        List<Article> localList = this.articleDao.findList(Integer.valueOf(i), Integer.valueOf(COUNT_NUMBER), null, null);
...
//      {
//        List<Product> localList = this.productDao.findList(Integer.valueOf(i), Integer.valueOf(COUNT_NUMBER), null, null);
...
@Transactional(readOnly=true)
  public Page<Article> search(String keyword, Pageable pageable)
  {
    ...
    try
    {
      ...
      FuzzyQuery localFuzzyQuery = new FuzzyQuery(new Term("title", str), WEIGHT);
      ...
      return new Page<Article>(localFullTextQuery.getResultList(), localFullTextQuery.getResultSize(), pageable);
    }
...
@Transactional(readOnly=true)
  public Page<Product> search(String keyword, BigDecimal startPrice, BigDecimal endPrice, Product.OrderType orderType, Pageable pageable)
  {
    ...
    try
    {
      ...
      FuzzyQuery localFuzzyQuery = new FuzzyQuery(new Term("name", str), WEIGHT);
      ...
      {
    	  Query localObject = NumericRangeQuery.newDoubleRange("price", Double.valueOf(startPrice.doubleValue()), Double.valueOf(endPrice.doubleValue()), true, true);
      ...
      {
    	  Query  localObject = NumericRangeQuery.newDoubleRange("price", Double.valueOf(startPrice.doubleValue()), null, true, false);
        localBooleanQuery2.add((Query)localObject, BooleanClause.Occur.MUST);
      }
      else if ((endPrice != null) && (endPrice.compareTo(new BigDecimal(0)) >= 0))
      {
    	  Query localObject = NumericRangeQuery.newDoubleRange("price", null, Double.valueOf(endPrice.doubleValue()), false, true);
        localBooleanQuery2.add((Query)localObject, BooleanClause.Occur.MUST);
      }
      ...
      localFullTextQuery.setFirstResult((pageable.getPageNumber() - 1) * pageable.getPageSize());
      localFullTextQuery.setMaxResults(pageable.getPageSize());
      return new Page<Product>(localFullTextQuery.getResultList(), localFullTextQuery.getResultSize(), pageable);
    }
