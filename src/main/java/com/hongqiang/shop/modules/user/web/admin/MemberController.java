...
//    model.addAttribute("memberRanks", this.memberRankService.findAll());
  ...
  public String save(Member member, Long memberRankId, HttpServletRequest request, RedirectAttributes redirectAttributes)
  {
    ...
    if (!beanValidator(redirectAttributes,member, new Class[] { BaseEntity.Save.class }))
      return ERROR_PAGE;
    ...
    while (localIterator.hasNext())
    {
      ...
      else
      {
        ...
        else if (localMemberAttribute.getType() == MemberAttribute.Type.birth)
        {
          try
          {
            Date localDate = StringUtils.isNotEmpty(str) ? DateUtils.parseDate(str, CommonAttributes.DATE_PATTERNS) : null;
            if ((localMemberAttribute.getIsRequired().booleanValue()) && (localDate == null))
              return ERROR_PAGE;
        ...
        else
        {
          ...
          {
            localObject1 = StringUtils.isNotEmpty(str) ? (Area)this.areaService.find(Long.valueOf(str)) : null;
          ...
          else
          {
            ...
            List localObject2 = localObject1 != null ? Arrays.asList(localObject1) : null;
...
//    model.addAttribute("memberRanks", this.memberRankService.findAll());
  ...
  public String update(Member member, Long memberRankId, Integer modifyPoint, BigDecimal modifyBalance, String depositMemo, HttpServletRequest request, RedirectAttributes redirectAttributes)
  {
    ...
    this.memberService.update(localMember, modifyPoint, modifyBalance, depositMemo, this.adminService.getCurrent());
    addMessage(redirectAttributes, ADMIN_SUCCESS);
    return (String)"redirect:list.jhtml";
  }
