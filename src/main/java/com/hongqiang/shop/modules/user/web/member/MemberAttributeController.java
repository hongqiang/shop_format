...
//    	addMessage(redirectAttributes, Message.warn("admin.memberAttribute.addCountNotAllowed", new Object[] { Integer.valueOf(10) }));
  ...
  {
    if (!beanValidator(redirectAttributes,memberAttribute, new Class[] { BaseEntity.Save.class }))
      return ERROR_PAGE;
    ...
    Object localObject = this.memberAttributeService.findUnusedPropertyIndex();
    if (localObject == null)
      return ERROR_PAGE;
  ...
  public String update(MemberAttribute memberAttribute, RedirectAttributes redirectAttributes)
  {
    if (!beanValidator(redirectAttributes,memberAttribute, new Class[0]))
      return ERROR_PAGE;
    MemberAttribute localMemberAttribute = (MemberAttribute)this.memberAttributeService.find(memberAttribute.getId());
...
//    this.memberAttributeService.update(memberAttribute, new String[] { "type", "propertyIndex" });
    addMessage(redirectAttributes, ADMIN_SUCCESS);
  ...
  public Message delete(Long[] ids)
  {
   ...
   public void doit(){
		...
		Pageable pageable = new Pageable(1,40);
  		Page<MemberAttribute> page=this.memberAttributeService.findPage(pageable);
